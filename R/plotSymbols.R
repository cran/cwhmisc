plotSymbols <- function(interactive = FALSE) {
  ...
  on.exit(par(opar))
  plot(i %% ncol, 1 + i %/% ncol,  pch = j - 1, xlim = c(0, ncol - 1),
  ...
  if (interactive) {
    ...
    while (!ready) {
      ...
      if (!ready) {
        ...
        cat("Selected ASCII character '", ch, "' ", dec, " 0x",
             hex, " \\", oct, "\n",sep = "")
      }
    }
    return(df)
...
plotSymbolsFonts <- function (fn = 1) {
    ...
    plot(i %% ncol, 1 + i %/% ncol, pch = i, font = fn, xlab = "",
          ylab = "", axes = FALSE)
    axis(1, at = 0:15)
    axis(2, at = 1:16, labels = 0:15 * 16, las = 2)
    axis(3, at = 0:15)
