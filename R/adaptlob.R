...
#   Reference: Gander, Computermathematik, Birkhaeuser, 1992.
  ...
  Q <- XXXadaptsimstp(f, term, a,b,fa,fm,fb,is,trace,...)
    if (trace) cat(a, b-a, Q$Q, Q$term,"\n")
  return(Q)
}  # end adaptsim


XXXadaptsimstp <- function(f,term,a,b,fa,fm,fb,is,trace,...) {
#XXXadaptsimstp  Recursive function used by adaptsim.
#
#   q <- XXXadaptsimstp('f',a,b,fa,fm,fb,is,trace) tries to
...
#   Reference: Gander, Computermathematik, Birkhaeuser, 1992.
  ...
  if(is == 0) is <- b-a
  Q <- XXXadaptlobstp(f,term, a,b,fa,fb,is,trace,...)
    if (trace) cat(a, b-a, Q$Q, Q$term,"\n")
  return(Q)
}  # end adaptlob

XXXadaptlobstp <- function(f,term,a,b,fa,fb,is,trace,...) {
#   Recursive function used by adaptlob.
#
#   Q  <-  XXXadaptlobstp('f',a,b,fa,fb,is,trace) tries to
#   approximate the integral of f(x) from a to b to
