...
LB2YX <- function(long, lat) {
  ...
  y5 <-  ye1 * P + ye0
  yt_e   <- ( (y5 * L2 + y3) * L2 + y1) * L * 1000.0 + yToEastBerne
  x0 <- ( ( ( ( (xc6 * P + xc5) * P  + xc4) * P + xc3) * P  + xc2) * P
            + xc1) * P
  x2 <- ( ( ( xd4 * P + xd3) * P  + xd2) * P + xd1) * P  + xd0
  x4 <- (xe2 * P + xe1) * P + xe0
  x6 <-  xf0
  xt_n  <- ( ( (x6 * L2 + x4) * L2 + x2) * L2 + x0) * 1000.0
             + xtoNorthBerne
  return(list(yE = yt_e, xN = xt_n))
}  ## end  LB2YX

YX2LB <- function(yt_e, xt_n) {
  if (missing(xt_n)) {
    xt_n <- yt_e[[2]]
    yt_e <- yt_e[[1]]
  }
